...
export const useTrainingStore = create((set) => ({
    ...
    approvalsQueue: [],
    completedTrainings: [],
    completeTraining: (assignmentId, record) => set((state) => {
        const assignment = state.pendingTrainings.find((item) => item.id === assignmentId);
        if (!assignment) {
            return state;
        }
        const newRecord = {
        ...
        return {
            pendingTrainings: state.pendingTrainings.filter((item) => item.id !== assignmentId),
            approvalsQueue: [...state.approvalsQueue, newRecord],
            completedTrainings: state.completedTrainings,
        };
    }),
    approveTraining: (recordId, supervisor) => set((state) => {
        ...
        return {
            ...
            approvalsQueue: state.approvalsQueue.filter((item) => item.id !== recordId),
