...
export default function UserManagementPage() {
  ...
  const useManualPassword = watch('useManualPassword');
  ...
  const handleInvite = (data: InviteForm) => {
    createUserMutation.mutate({
      ...
      password: data.useManualPassword ? data.password : undefined
    });
  };
  ...
  const handleEdit = (user: any) => {
    ...
    setValue('password', '');
    setValue('useManualPassword', false);
    setOpenActionMenuId(null);
    setEditOpen(true);
  };

  const handleUpdate = async (data: InviteForm) => {
    ...
    if (data.password && data.password.length >= 8) {
      try {
        await api.post(`/users/${editingUser.id}/reset-password`, { newPassword: data.password });
      } catch (error: any) {
      ...
      >
        <form className="space-y-6" onSubmit={handleSubmit(handleInvite)}>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-purple-200 bg-purple-50 p-4 dark:border-purple-500/40 dark:bg-purple-500/10">
              <label className="flex items-start gap-3">
                <input
                  type="checkbox"
                  {...register('useManualPassword')}
                  className="mt-0.5 h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary"
                />
                <div>
                  ...
                  <p className="mt-1 text-xs text-purple-700 dark:text-purple-200">
                    By default, a secure random password will be generated. Check this to set a custom password.
                  </p>
                </div>
              </label>
            </div>
      ...
      >
        <form onSubmit={handleSubmit(handleUpdate)} className="space-y-6">
          ...
          <div className="space-y-4 border-t border-slate-200 pt-4 dark:border-slate-700">
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/40 dark:bg-amber-500/10">
              <p className="text-sm font-semibold text-amber-900 dark:text-amber-100">🔐 Reset Password (Optional)</p>
              <p className="mt-1 text-xs text-amber-700 dark:text-amber-200">
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Leave blank to keep existing password. Minimum 8 characters if changing.
              </p>
            </div>
          </div>
