...
export async function login(identifier, password, locationCode) {
    const normalizedIdentifier = identifier.toLowerCase();
    const normalizedLocationCode = locationCode.toUpperCase();
    const query = await pool.query(`SELECT u.id,
     ...
     WHERE (u.login_identifier = $1 OR u.email = $1)
       AND l.code = $2`, [normalizedIdentifier, normalizedLocationCode]);
