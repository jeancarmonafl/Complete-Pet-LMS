...
async function fetchNextEmployeeSequence(locationId) {
    const result = await pool.query(`SELECT COALESCE(MAX(CAST(SUBSTRING(employee_id FROM 4) AS INTEGER)), 0) + 1 AS next_sequence
     FROM users
...
export async function createUser(payload) {
    ...
    try {
        ...
        const insertResult = await client.query(`INSERT INTO users (
        organization_id,
        location_id,
        full_name,
        login_identifier,
...
export async function listSupervisors(locationId) {
    return pool.query(`SELECT id, full_name AS fullName
     FROM users
     WHERE location_id = $1 AND app_role IN ('supervisor', 'manager', 'admin', 'global_admin')
     ORDER BY full_name`, [locationId]);
}
export async function listUsers(locationId) {
...
export async function updateUser(userId, locationId, updates) {
    const fields = [];
    const values = [];
    let paramCount = 1;
    const fieldMap = {
...
export async function updateUserStatus(userId, locationId, isActive) {
    return pool.query(`
      UPDATE users
...
export async function getUserActivity(userId, locationId) {
    const client = await pool.connect();
    try {
