...
export async function listSupervisorsHandler(req, res) {
    if (!req.user) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
...
export async function getUserActivityHandler(req, res) {
    if (!req.user) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
    try {
        const userId = req.params.id;
        if (!['global_admin', 'admin', 'manager', 'supervisor'].includes(req.user.role)) {
            return res.status(403).json({ message: 'Forbidden' });
        }
        const activity = await getUserActivity(userId, req.user.locationId);
        if (!activity) {
            return res.status(404).json({ message: 'User not found' });
        }
