export function buildEmployeeId(latestNumericId: number): string {
  return `EMP${`${latestNumericId}`.padStart(3, '0')}`;
}

export function buildLoginIdentifier(fullName: string, employeeId: string, email?: string | null): string {
